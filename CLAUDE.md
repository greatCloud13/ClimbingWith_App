...
lib/
  ...
  features/
    ...
    record/          # record_screen.dart — 최근 완등 기록(목업, 통계·랭킹은 숨김 처리됨)
...
- **API**: `POST /api/auth/signup` `{username, nickname, email, password}` → `{message}` / `POST /api/auth/login` `{username, password}` → `{userId, token, username, role, nickname, managedGymId}`
- **유효성 검증**: 백엔드 DTO 제약과 동일하게 클라이언트에서도 검증 — username/nickname 4~20자, password 8~20자, email 형식
- **세션 저장**: 로그인 성공 시 토큰 + 사용자 정보(userId/username/nickname/role/managedGymId)를 `flutter_secure_storage`에 함께 저장. **주의**: 백엔드에 세션 복원용 "내 정보 조회"(`/api/auth/me` 등) API가 아직 없어서, 앱 재시작 시 로그인 응답에서 받은 사용자 정보를 그대로 복원하는 방식으로 구현함 (네트워크 재검증 없음). 저장된 토큰이 실제로는 만료됐더라도 앱은 우선 로그인된 것처럼 보여주고, 이후 첫 인증 필요 API 호출이 401을 받으면 그때 강제 로그아웃된다. `/api/auth/me`가 생기면 앱 시작 시 그 API로 세션을 검증하는 방식으로 교체 권장.
  - **`userId` 추가 (2026-08-18)**: 로그인 응답에 숫자 `userId`가 추가되어 `CurrentUser.userId`(nullable)로 저장함 — 이 필드가 없던 시절에 저장된 세션은 `null`로 복원되고 재로그인해야 채워진다. 이 값으로 `GET /api/problemTryLog/user/{userId}`(트라이 기록 이력), `GET /api/clearRecord/user/{userId}/problem/{problemId}/inProgress`(진행 중 완등 기록 재사용) 연동 완료 — 자세한 내용은 `reports/2026-08-10_problem-detail-stats-api-request.md` 참고.
...
**로그인 없이도 앱 사용 가능(게스트 모드)**: 앱을 켰을 때 첫 화면은 로그인이 아니라 홈이다. `/profile`, `/record`, `/gym-manage/board`(`/write` 포함)만 로그인을 요구하고(`_authRequiredPaths`, [app_router.dart](lib/core/router/app_router.dart)), `/home`·`/gym`·`/more`는 게스트도 접근 가능. 홈 화면 안에서 개인화 섹션(즐겨찾기 클라이밍장)은 로그인 상태일 때만 보이고, 비로그인 시 그 자리에 로그인 유도 카드(`_GuestPromptCard`)가 대신 표시된다.

- `/home` — [home_screen.dart](lib/features/home/presentation/home_screen.dart): "오늘은 어느 암장으로 가실건가요?" 헤더 + 알림 버튼(공개). 로그인 시에만: 즐겨찾기 클라이밍장 **세로 피드** — 카드 하나가 [암장이름 + 메인 사진 + 그 암장에 종속된 공지]로 구성됨. **공지는 전역이 아니라 암장별로 붙는다** — 처음엔 전역 공지 슬라이드로 잘못 구현했다가 사용자 와이어프레임을 받고 암장별 종속 구조로 재작업함.
  ...
  - **연속방문 스트릭 / 친구 활동 숨김 처리 (2026-08-18)**: API 없이 목업으로만 있던 두 섹션(`_StreakStatCard`, `_FriendActivityCard`)을 사용자 확인 하에 화면에서 제거함 — 목업 데이터 파일(`home_mock_data.dart`, `friend_activity.dart`)도 함께 삭제. 실제 API 연동 시점에 다시 추가 예정.
...
- `/record` — [record_screen.dart](lib/features/record/record_screen.dart). **이번달 완등/최고 난이도/연속 방문 통계, 이번주 랭킹은 숨김 처리 (2026-08-18)**: 전부 API 없이 목업이던 부분이라 사용자 확인 하에 제거하고 "최근 완등 기록"만 남김 — 실제 API 연동 시점에 다시 추가 예정.
  - **"최근 완등 기록" 실연동 완료 (2026-08-19)**: `GET /api/clearRecord/user/{userId}`(최신순, `size=10`)로 교체. 응답의 `problemId`/`gymId`/`sectorId`로 카드를 탭하면 문제 상세 화면(`/gym/:id/sector/:sectorId/problem/:problemId`)까지 정확히 이동한다 — 처음 요청 시 `problemId`가 빠져 있어 카드 탭 네비게이션을 만들지 못하고 대기했다가, 추가된 뒤 진행함(`reports/2026-08-19_recent-clear-records-api-request.md` 참고). 난이도는 V등급이 아니라 암장별 커스텀 레벨(`level`/`levelColorCode`)이라 `GradeBadge` 대신 다른 화면들과 같은 색 스와치+이름 방식으로 표시. `/record`는 로그인 필수 라우트라 게스트 분기는 없지만, `userId`가 없는 구 세션(재로그인 전)은 안내 문구로 대체. [record_api.dart](lib/features/record/data/record_api.dart) / [record_providers.dart](lib/features/record/application/record_providers.dart).
...
- **문제 상세조회 화면 구현 (2026-08-10)**: 문제 목록의 카드를 탭하면 [problem_detail_screen.dart](lib/features/gym/presentation/problem_detail_screen.dart)(`/gym/:id/sector/:sectorId/problem/:problemId`)로 이동. 볼더/리드 모두 같은 구성(홀드 갯수·내 트라이 기록·낙하 지점 분포·클리어 인원)을 쓴다 — 볼더 전용 스펙은 아직 없어 사용자 확인 하에 리드와 동일하게 채움. 백엔드 확인: "홀드"는 서버 데이터 모델에 없는 개념이고 `Problem.holdCount` 기준 1~N 순번("몇 번째 홀드")으로만 다룬다.
  - **루트 토포**: `CustomPainter`로 홀드를 순서대로 그리고 커뮤니티 낙하 지점을 색 진하기(히트맵)로 표시. **리드는 루트가 길어질 수 있어 좌(시작)→우(완등)로 매 10개씩 줄바꿈(1~10/11~20/21~30 …이 세로로 쌓임), 볼더는 하(시작)→상(완등) 세로 단일 컬럼**으로 배치가 다르다([gym_problem.dart](lib/features/gym/domain/gym_problem.dart)의 `problemType`으로 분기) — 처음엔 리드를 가로 스크롤 한 줄로 만들었다가, 옆으로 계속 늘어놓지 말고 구간마다 줄을 바꾸라는 피드백을 받고 줄바꿈형으로 교체함(가로 스크롤 제거, `LayoutBuilder`로 한 줄에 10홀드가 항상 화면 폭에 맞게 들어가도록 간격을 동적 계산). 두 배치 모두 구간 경계에는 연결선을 긋지 않고 각 줄 시작에 "1~10" 같은 라벨을 그린다.
  - **버그 수정 (2026-08-10)**: 커뮤니티 낙하 지점 히트맵이 처음 진입 시 목업 색으로 보였다가 화면 크기를 바꿔야만 실데이터로 바뀌는 버그가 있었음 — `_RouteTopoPainter.shouldRepaint`가 `communityFallCounts`(홀드별 낙하 인원 맵) 변경을 비교 대상에서 빠뜨려서, `dropPointStatsProvider`가 늦게 응답해도 `CustomPaint`가 다시 그리지 않은 게 원인. `shouldRepaint`에 `mapEquals`/`listEquals` 비교를 추가해 해결.
  - **홀드 갯수 / 내 트라이 / 내 최고 도달 / 클리어 인원 / 커뮤니티 낙하 지점 분포는 실연동 완료 (2026-08-10)**: `GET /api/problem/{id}/detail`([gym_problem_detail.dart](lib/features/gym/domain/gym_problem_detail.dart))의 `holdCount`/`myTryCount`(로그인 사용자 개인 시도 횟수 — 백엔드에서 필드명을 `tryCount`에서 `myTryCount`로 바꿔줌)/`myBestDropPoint`/`clearCount`, 그리고 `GET /api/problem/{id}/dropPointStats`([drop_point_stats.dart](lib/features/gym/domain/drop_point_stats.dart))의 `distribution`(홀드 순번별 낙하 인원)을 그대로 쓴다. 통계 카드 4개(홀드 갯수·내 트라이·내 최고 도달·클리어 인원)로 표시. **게스트 체험 모드는 `holdCount`가 목업이라 dropPointStats도 실데이터 대신 계속 문제 id 기준 목업**([problem_try_mock_data.dart](lib/features/gym/data/problem_try_mock_data.dart))을 쓴다 — 실제/목업 홀드 갯수가 어긋나면 낙하 지점이 안 맞을 수 있어서.
  - **트라이 시작 / 여기서 떨어짐 실제 동작 — `problemTryLog` API 연동 완료 (2026-08-10)**: "트라이 시작" → 루트 토포를 탭해 도달 지점을 고르는 모드 진입(선택 전엔 흰색 점선 링으로 "선택 중" 표시) → "여기서 떨어짐"(최상단 홀드를 고르면 버튼 라벨이 "완등으로 기록"으로 바뀜)을 누르면 확인 모달이 뜨고, 모달에서 "어떤 홀드였나요?" 메모(선택 입력)를 받은 뒤 "기록하기"로 확정하면 `POST /api/problemTryLog`가 호출된다(성공 시 문제 상세를 재조회해 서버가 다시 계산한 `myTryCount`/`myBestDropPoint` 반영. `tryDate`는 클라이언트가 뭘 보내도 서버가 항상 오늘 날짜로 저장한다고 확인받아 요청 본문에서 뺌). "취소"로 중단 가능. [problem_try_providers.dart](lib/features/gym/application/problem_try_providers.dart)의 `StateNotifierProvider.family`(문제 id 기준). **"점장님의 팁"(홀드별 공략 조언, 목업)은 매니저 작성 기능이 없어 사용자 확인 하에 숨김 처리(2026-08-19)** — `_ManagerTipCard`와 `mockManagerTips` 제거함.
  - **비로그인 게스트 = "체험 모드" (2026-08-10)**: `GET /api/problem/{id}/detail`은 게스트에게 401이 아니라 **403**을 준다고 확인받음 — 이 화면(`/gym/:id/sector/:sectorId/problem/:problemId`)은 게스트도 들어올 수 있는 라우트라, `gymProblemDetailProvider`가 403을 받으면 공개 API `GET /api/problem/{id}`(인증 불필요)로 대체 조회하고 서버가 못 주는 `holdCount`만 문제 id 기준 목업으로 채운 "게스트 체험" 데이터(`GymProblemDetail.isGuestPreview`)를 대신 쓴다([gym_providers.dart](lib/features/gym/application/gym_providers.dart)). 화면 상단에 "게스트 체험 모드" 배너를 띄우고, 이 상태에서의 "트라이 시작"/"여기서 떨어짐"은 `POST /api/problemTryLog`를 호출하지 않고 `ProblemTryLog.isLocalOnly=true`로 세션 로컬에만 기록 — 화면을 나가면 사라진다(통계 카드의 내 트라이/내 최고 도달도 이 로컬 기록으로 계산).
  - **"트라이 기록" 과거 이력 실연동 완료 (2026-08-18)**: 로그인 응답에 숫자 `userId`가 추가돼 `GET /api/problemTryLog/user/{userId}`(최근 100건, 문제별 필터는 없어 클라이언트에서 `problemId`로 걸러 씀)로 과거 기록을 보여준다(`myProblemTryLogsProvider`, [gym_providers.dart](lib/features/gym/application/gym_providers.dart)) — 새 트라이 기록 시 자동 갱신. `userId`가 없는 구 세션(이 변경 전 로그인)은 재로그인 전까지 세션 로컬 기록만 보이고 안내 문구가 뜬다. 게스트는 여전히 세션 로컬(체험 모드).
  - "내 기록"은 텍스트 요약이 아니라 트라이 기록 리스트(`_TryHistoryList`, 최신순 카드: 날짜 + 도달/완등 결과 + 메모)로 표시 — 기록이 메인 기능이라는 피드백 반영.
  - **완등 기록(ClearRecord) 연동 (2026-08-10, 부분)**: `problemTryLog`(개별 낙하 지점)와는 별개로 "언제부터 도전해서 언제 완등했는지"를 담당하는 `ClearRecord`가 추가됨. "트라이 시작"을 누르면 이 화면에 머무는 동안 한 번만 `POST /api/clearRecord`(진행 중, `isClear=false`)를 만들고 완등 전까지 재사용, 완등(dropPoint == holdCount)으로 기록하면 `PATCH /api/clearRecord/{id}/clear`로 완등 처리. 완등 인증 영상(`videoUrl`) 입력과 다른 사람 완등 영상 갤러리(`existVideo` API)는 이번 범위에서 제외(사용자 확인).
  - **통계 카드에 트라이 시작일자 / 완등일자 추가 (2026-08-18)**: 통계 카드가 4개(홀드 갯수·내 트라이·내 최고 도달·클리어 인원)에서 2줄 6개로 늘어남. 처음엔 `ClearRecord`(진행 중 조회 API + 이번 세션 완등 응답)로 구현했으나, **트라이 기록(`tryHistory`, `GET /api/problemTryLog/user/{userId}`)에서 직접 구하는 방식으로 교체**함 — `problemTryLog`는 `problemId`가 명확해 이름 매칭 오류가 없고, 과거 세션에 완등한 기록도(최근 100건 안에서는) 잡힌다는 게 `ClearRecord` 조회 API 대비 장점. **시작일자** = 트라이 기록 중 가장 오래된 날짜(없으면 "미시작"), **완등일자** = `dropPoint >= holdCount`인 기록 중 가장 최근 날짜(없으면 "미완등") — [problem_detail_screen.dart](lib/features/gym/presentation/problem_detail_screen.dart)의 `_startDateLabel`/`_clearDateLabel`.
  - **진행 중 기록 재사용 실연동 완료 (2026-08-18)**: 로그인 응답에 숫자 `userId`가 추가돼 `GET /api/clearRecord/user/{userId}/problem/{problemId}/inProgress`(`GymApi.fetchInProgressClearRecord`)로 화면 재방문 시에도 기존 진행 중 기록을 먼저 찾아 재사용하고, 없을 때만 새로 만든다(`ProblemTryNotifier.startTry`). `userId`를 모르는 구 세션은 조회를 건너뛰고 바로 생성.
