...
- **문제 상세조회 화면 구현 (2026-08-10)**: 문제 목록의 카드를 탭하면 [problem_detail_screen.dart](lib/features/gym/presentation/problem_detail_screen.dart)(`/gym/:id/sector/:sectorId/problem/:problemId`)로 이동. 볼더/리드 모두 같은 구성(홀드 갯수·내 트라이 기록·낙하 지점 분포·클리어 인원)을 쓴다 — 볼더 전용 스펙은 아직 없어 사용자 확인 하에 리드와 동일하게 채움. 백엔드 확인: "홀드"는 서버 데이터 모델에 없는 개념이고 `Problem.holdCount` 기준 1~N 순번("몇 번째 홀드")으로만 다룬다.
  ...
  - **"트라이 기록" 리스트는 이번 세션에 새로 만든 것만 보임 — 과거 기록 조회는 미해결**: `GET /api/problemTryLog/user/{userId}`만 있고 (1) 로그인 응답에 숫자 `userId`가 없고 (2) 이 API가 문제별 필터 없이 사용자 전체 기록을 섞어 줘서, 현재는 이 화면에서 (로그인 사용자라도) 과거 기록을 불러올 방법이 없다. 화면엔 상황에 맞는 안내 문구(게스트: "저장되지 않아요", 로그인: "이번 세션에 새로 기록한 트라이만 보여요")를 넣어뒀고, `reports/2026-08-10_problem-detail-stats-api-request.md`에 `userId` 노출 또는 문제별 조회 API를 요청해둠.
